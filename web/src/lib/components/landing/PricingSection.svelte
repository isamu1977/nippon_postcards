<script lang="ts">
  export let pricingInfo = {
    title: "Simple, transparent pricing",
    description: "Each postcard costs US$ 15 with mailing included.",
...
<section id="pricing" class="py-16 sm:py-20 bg-gray-50">
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="max-w-2xl mx-auto text-center mb-8">
      <h2 class="text-3xl sm:text-4xl font-bold text-gray-900">{pricingInfo.title}</h2>
      <p class="text-gray-600 mt-2">{pricingInfo.description}</p>
    </div>

    <div class="max-w-md mx-auto">
      <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-10 text-center">
        <div class="text-sm text-gray-500">Price</div>
        <div class="mt-3 text-4xl sm:text-5xl font-extrabold text-gray-900">US$ 15 <span class="text-sm font-medium text-gray-600">per postcard</span></div>

        <div class="mt-6 text-sm text-gray-600">
          <ul class="list-disc list-inside space-y-2">
            {#each pricingInfo.additionalNotes as note}
              <li>{note}</li>
            {/each}
          </ul>
        </div>
